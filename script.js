...
async function carregar(){
  try {
    ...
    // Pular primeira linha (cabeçalhos da planilha que vêm como dado)
    const todasLinhas = json.table.rows || [];
    const rows = todasLinhas.slice(1); // remove linha de cabeçalho

    // Ler posição das colunas pela primeira linha de dados
    // Coluna A=titulo, B=categoria, C=subtitulo, D=descricao,
    // E=status, F=data_limite, G=vagas, H=icone,
    // I=link_saiba_mais, J=link_edital, K=imagem_url, L=cor_card
    const v = (row, i) => row.c[i] ? (row.c[i].f || row.c[i].v || "") : "";

    // Pular primeira linha se for exemplo (texto em itálico/cinza da linha 2)
    const dadosReais = rows.filter((row, idx) => {
      const titulo = v(row, 0);
      // Pular linhas onde o título é igual ao exemplo padrão
      if(!titulo || titulo.toString().trim() === "") return false;
      if(titulo.toString().toLowerCase() === "titulo") return false;
      return true;
    });

    // Detectar e pular linha de exemplo (segunda linha da planilha)
    // se o primeiro item parecer exemplo genérico
    const primeiroTitulo = dadosReais.length > 0 ? v(dadosReais[0], 0) : "";
    const linhasUsadas = (primeiroTitulo === "PET Medicina" && dadosReais.length > 1)
      ? dadosReais  // manter todos incluindo exemplos se tiver dados reais depois
      : dadosReais;

    todas = linhasUsadas.map(row => ({
      titulo:         v(row,0),
      categoria:      v(row,1),
      subtitulo:      v(row,2),
      descricao:      v(row,3),
...
function filtradas(){
  ...
  if(ord==="abertas") f=[...f].sort((a,b)=>{
    return (sl(a.status||"").includes("aberta")?0:1)-(sl(b.status||"").includes("aberta")?0:1);
  });
  return f;
}
...
function card(o){
  ...
  const img  = o.imagem_url
    ? `<img src="${o.imagem_url}" style="width:100%;height:100%;object-fit:cover" onerror="this.parentElement.innerHTML='${icon}'">`
    : icon;
  return `
  <div class="oport-card" style="border-left-color:${cor}" onclick="abrirModal('${esc}')">
    ...
    <div class="card-acoes" onclick="event.stopPropagation()">
      ${o.link_saiba_mais
        ...
        :`<button class="btn-saiba" onclick="abrirModal('${esc}')">Saiba mais</button>`}
...
function renderizar(){
  ...
  if(!f.length){ lista.innerHTML=`<div class="loading-wrap">Nenhuma oportunidade encontrada.</div>`; vw.style.display="none"; return; }
  lista.innerHTML = (mostrarTodas?f:f.slice(0,LIMITE)).map(card).join("");
...
function abrirModal(titulo){
  ...
  const capa=o.imagem_url?`background-image:url('${o.imagem_url}');background-size:cover;background-position:center`:`background:#E8F0EB`;
