...
async function carregar(){
  try {
    ...
    // Pular linhas de exemplo (linha 4 da planilha que contém textos "Ex:")
    const primeiraLinhaValida = json.table.rows.findIndex(row => {
      const celula = row.c[0];
      const val = celula ? String(celula.f || celula.v || "") : "";
      return val.trim() !== "" && !val.startsWith("Ex:");
    });

    const linhasValidas = primeiraLinhaValida >= 0
      ? json.table.rows.slice(primeiraLinhaValida)
      : json.table.rows;

    todas = linhasValidas.map(row => {
      const obj = {};
      cols.forEach((col,i) => { const cell=row.c[i]; obj[col]=cell?(cell.f||cell.v||""):""; });
      return obj;
    }).filter(o => o.titulo && o.titulo.toString().trim() !== "" && !o.titulo.toString().startsWith("Ex:"));
