...
const CONFIG = {
  SHEET_ID:   "1CP4jbGV06TGbjodwGnwXvS6T3AuWLg37D0pBVvWt-0I",
...
async function carregar(){
  try {
    ...
    // Usar labels das colunas como chaves
    ...
    todas = (json.table.rows||[]).map(row => {
      ...
      const t = (o.titulo || "").toString().trim();
      return t !== "" && t !== "null";
    });
    ...
    document.getElementById("oport-lista").innerHTML =
      `<div class="loading-wrap">⚠️ Não foi possível carregar as oportunidades.<br><small>${e.message}</small></div>`;
  }
}
...
function card(o){
  ...
  const tituloEsc = (o.titulo||"").replace(/'/g,"\\'").replace(/"/g,'&quot;');
  const imgHtml = o.imagem_url
    ? `<img src="${o.imagem_url}" alt="${o.titulo}" style="width:100%;height:100%;object-fit:cover" onerror="this.parentElement.innerHTML='${icon}'">`
    : icon;
...
function renderizar(){
  ...
  lista.innerHTML = ex.map(card).join("");
